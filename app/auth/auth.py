...
class Auth:
    ...
    async def get_current_user(self, token: HTTPAuthorizationCredentials = Depends(HTTPBearer()), db: AsyncSession= Depends(get_db)):
        ...
        user_result = await db.execute(select(User).where(User.id == token.id))
